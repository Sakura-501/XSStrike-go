...
func runSingleScan(opts *options.Options, headers map[string]string, client *requester.Client) {
	...
	if opts.PayloadFile != "" {
		...
		bfReport, err := bruteforce.RunWithConfig(client, opts.URL, opts.Data, opts.JSON, opts.Path, headers, payloadList, opts.Encode, bruteforce.Config{Threads: opts.ThreadCount})
		if err != nil {
